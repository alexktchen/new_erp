...
| 5 | admin「今日待確認」（每日進入點 + 早上 LINE 推提醒） | 確保老闆每天知道要做什麼 |
...
3. **「7 天視窗外的折疊」存法**：用 status flag（`status='archived_by_age'`）還是純前端 filter？建議 cron 每日掃、超過 X 天標 status，UI 過濾。具體 X 預設多少（30 / 60 / 90？）。
4. **「商品週曆」是新表還是新欄位**？建議候選池表加 `scheduled_open_at DATE`、週曆 UI 純讀這欄位、不開新表。
5. **相似商品提示的演算法**：MVP 用品名 LIKE / trigram 模糊比對；之後可升級成 embedding（issue #103 順便用）。
6. **「採用建正式商品」的最小欄位集**：候選池要預填哪些 product 欄位（品名 / 圖 / 參考價肯定預填）、老闆採用當下最少要補什麼（建議只補「分類」、其他 default）。需 Alex 確認。
7. **機器人密鑰放哪**：Supabase Vault / 環境變數 / Edge Function secret？延續既有做法即可。
8. **「今日待確認」LINE 提醒時段**：早上幾點推？預設 8:00 嗎？

---

## 8. 後續可轉 PRD 的內容

當 Alex 把 §7 八題答完後，這份可升 `PRD-社群商品候選池.md`，需要展開的內容：
